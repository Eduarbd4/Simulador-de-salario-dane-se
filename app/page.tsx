...
export default function Home(){
  ...
  if(role==="seller"&&!store)return <main className="login-page brasilia-login"><section className="login-card brasilia-login-card"><div className="login-form-area"><button className="back-link" onClick={()=>setRole(null)}>← Voltar</button><div className="danese-login-brand">DANE SE</div><p className="eyebrow">Vendedores · acesso por loja</p><h1>Bem-vinda(o)</h1><p>Digite a senha da sua loja.</p><label>Senha<input type="password" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==="Enter"&&enter()} autoFocus/></label>{error&&<div className="login-error">{error}</div>}<button onClick={enter} disabled={loading}>{loading?"Entrando...":"ACESSAR  →"}</button></div><div className="login-art" aria-hidden="true"><div className="tile-grid login-tiles">{Array.from({length:8}).map((_,i)=><i key={i}/>)}</div><p>BRASÍLIA<br/>DANE SE</p></div></section></main>;

  if(role==="sub"&&!subLogged)return <main className="login-page brasilia-login"><section className="login-card brasilia-login-card"><div className="login-form-area"><button className="back-link" onClick={()=>setRole(null)}>← Voltar</button><div className="danese-login-brand">DANE SE</div><p className="eyebrow">Sub Gerentes · acesso restrito</p><h1>Bem-vinda(o)</h1><p>Digite a senha de Sub Gerente.</p><label>Senha<input type="password" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==="Enter"&&enter()} autoFocus/></label>{error&&<div className="login-error">{error}</div>}<button onClick={enter} disabled={loading}>{loading?"Entrando...":"ACESSAR  →"}</button></div><div className="login-art" aria-hidden="true"><div className="tile-grid login-tiles">{Array.from({length:8}).map((_,i)=><i key={i}/>)}</div><p>BRASÍLIA<br/>DANE SE</p></div></section></main>;
  ...
  </div><footer><button className="logout" onClick={resetAll}>Sair</button></footer></div></main>;
}
